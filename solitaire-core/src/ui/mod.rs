...
pub fn build_solitaire_app() -> (App, SharedModel) {
    ...
    // automatic when the viewport changes. The Performance window
    // lives just under HelpDialog so its title bar is reachable above
    // the menu / HUD but a help overlay still scrims over it.
    //
